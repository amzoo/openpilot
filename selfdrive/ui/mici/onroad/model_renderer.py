...
class ModelRenderer(Widget, ModelRendererSP):
  ...
  def _update_lead_vehicle(self, d_rel, v_rel, point, rect):
    ...
    style = ui_state.chevron_style

    if style == 2:  # Open V
      apex = (x, y)
      left = (x - (sz * 1.25), y + sz)
      right = (x + (sz * 1.25), y + sz)
      g_apex = (x, y - g_yo)
      g_left = (x - (sz * 1.35) - g_xo, y + sz + g_yo)
      g_right = (x + (sz * 1.35) + g_xo, y + sz + g_yo)
      arm_r = sz * 0.15
      glow = [_capsule_vertices(g_apex, g_left, arm_r), _capsule_vertices(g_apex, g_right, arm_r)]
      chevron = [_capsule_vertices(apex, left, arm_r), _capsule_vertices(apex, right, arm_r)]
    ...
    return LeadVehicle(glow=glow, chevron=chevron, chevron_pos=(x, y), fill_alpha=int(fill_alpha))
